...
GHOST_COLORS_origin = []
GHOST_COLORS_origin.append(formatColor(.9,0,0)) # Red
GHOST_COLORS_origin.append(formatColor(0,.3,.9)) # Blue
GHOST_COLORS_origin.append(formatColor(.98,.41,.07)) # Orange
GHOST_COLORS_origin.append(formatColor(.1,.75,.7)) # Green
GHOST_COLORS_origin.append(formatColor(1.0,0.6,0.0)) # Yellow
GHOST_COLORS_origin.append(formatColor(.4,0.13,0.91)) # Purple
...
class PacmanGraphics:
  ...
  def drawAgentObjects(self, state):
    ...
    for index, agent in enumerate(state.agentStates):
      ...
      agentPosition_3 = state.agentStates[3].getPosition()
      agentPosition_1 = state.agentStates[1].getPosition()
      # print("agent的位置是:"+str(agentPosition))
      if (abs(agentPosition[0] - agentPosition_3[0]) + abs(agentPosition[1] - agentPosition_3[1]) > 6) and \
              (abs(agentPosition[0] - agentPosition_1[0]) + abs(agentPosition[1] - agentPosition_1[1]) > 6):
          if agent.isPacman:
              image = self.drawPacman_tan_black(agent, index)
              self.agentImages.append((agent, image))
          else:
              image = self.drawGhost_tan_black(agent, index)
              self.agentImages.append((agent, image))
      else:
          if agent.isPacman:
  ...
  def deleteImage(self, agentIndex):
    """
      Changes an image from a ghost to a pacman or vis versa (for capture)
    """
    prevState, prevImage = self.agentImages[agentIndex]
  ...
  def update(self, newState):
    ...
    self.index_current = agentIndex
    agentState = newState.agentStates[agentIndex]
    agentPosition = agentState.getPosition()
    agentPosition_3 = newState.agentStates[3].getPosition()
    agentPosition_1 = newState.agentStates[1].getPosition()
    if agentIndex not in [1, 3]:
        if (abs(agentPosition[0] - agentPosition_3[0]) + abs(agentPosition[1] - agentPosition_3[1]) > 5) and\
                (abs(agentPosition[0] - agentPosition_1[0]) + abs(agentPosition[1] - agentPosition_1[1]) > 6):
            GHOST_COLORS[agentIndex] = BACKGROUND_COLOR
        else:
            GHOST_COLORS[agentIndex] = GHOST_COLORS_origin[agentIndex]
    ...
    if newState._foodEaten != None:
      self.removeFood(newState._foodEaten, self.food)
        ...
        position_int = (int(agentPosition[0]), int(agentPosition[1]))
        if newState.food[position_int[0]][position_int[1]]:
            # print("踩到食物了，但都是自己的所以没吃")
            if GHOST_COLORS[agentIndex] == BACKGROUND_COLOR:
              self.deleteImage(agentIndex)
            # self.food = self.drawFood(self.food)
            # self.food = self.drawFood(newState.layout.food)
            # print(newState.layout.food)
    if newState._capsuleEaten != None:
      self.removeCapsule(newState._capsuleEaten, self.capsules)
    else:
      position_int = (int(agentPosition[0]), int(agentPosition[1]))
      if position_int in newState.capsules:
        # print("踩到了自己的胶囊")
        if GHOST_COLORS[agentIndex] == BACKGROUND_COLOR:
          self.deleteImage(agentIndex)
      # print(newState.capsules)
    # dumping food
  ...
  def drawPacman(self, pacman, index): #new state, agentIndex
  ...
  def drawPacman_tan_black(self, pacman, index):
    position = self.getPosition(pacman)
    screen_point = self.to_screen(position)
    endpoints = self.getEndpoints(self.getDirection(pacman))
  ...
  def drawGhost(self, ghost, agentIndex):
    ...
    if dir == 'West':
      dx = -0.2
    leftEye = circle((screen_x+self.gridSize*GHOST_SIZE*(-0.3+dx/1.5), screen_y-self.gridSize*GHOST_SIZE*(0.3-dy/1.5)), self.gridSize*GHOST_SIZE*0.2, colour, colour)
    rightEye = circle((screen_x+self.gridSize*GHOST_SIZE*(0.3+dx/1.5), screen_y-self.gridSize*GHOST_SIZE*(0.3-dy/1.5)), self.gridSize*GHOST_SIZE*0.2, colour, colour)
    leftPupil = circle((screen_x+self.gridSize*GHOST_SIZE*(-0.3+dx), screen_y-self.gridSize*GHOST_SIZE*(0.3-dy)), self.gridSize*GHOST_SIZE*0.08, colour, colour)
    rightPupil = circle((screen_x+self.gridSize*GHOST_SIZE*(0.3+dx), screen_y-self.gridSize*GHOST_SIZE*(0.3-dy)), self.gridSize*GHOST_SIZE*0.08, colour, colour)
    ghostImageParts = []
  ...
  def drawGhost_tan_black(self, ghost, agentIndex):
    pos = self.getPosition(ghost)
    dir = self.getDirection(ghost)
    (screen_x, screen_y) = (self.to_screen(pos) )
    coords = []
  ...
  def moveGhost(self, ghost, ghostIndex, prevGhost, ghostImageParts):
    ...
    if ghost.scaredTimer > 0:
        if GHOST_COLORS[ghostIndex] == BACKGROUND_COLOR:
